...
function sell($stockacc)
{
    ...
    $ch = validInt(Outility::getInt(), 1, count($account));
    echo $account[$ch - 1]->name . " selected!\nEnter Number Of Shares To Sell of " . $account[$ch - 1]->name . " : ";
    $qt = validInt(Outility::getInt(), 1, $account[$ch - 1]->quantity);
    //removing the stock
    ...
    print_r($stack);
    print_r($queue);
    echo "sold $qt shares successfully";function validInt($int, $min, $max)
{
    while (filter_var($int, FILTER_VALIDATE_INT, array("options" => array("min_range" => $min, "max_range" => $max))) === false) {
        echo ("Variable value is not within the legal range\n");
        echo "enter again : ";
    ...
    fscanf(STDIN, "%s\n");
    $stockacc->account = $account;
    $stockacc->stack = $stack;function validInt($int, $min, $max)
    {
        while (filter_var($int, FILTER_VALIDATE_INT, array("options" => array("min_range" => $min, "max_range" => $max))) === false) {
            echo ("Variable value is not within the legal range\n");
            echo "enter again : ";
